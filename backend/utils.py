...
async def predict_image(file):
    # Baca upload file jadi numpy array
    contents = await file.read()
    pil_image = Image.open(BytesIO(contents)).convert("RGB")
    image_np = np.array(pil_image)

    # Preprocess pakai Mediapipe Pose + Crop bounding box
    ...
    if results.pose_landmarks:
        ...
        # Map ke huruf A-Z
        predicted_number = le.classes_[pred_index]  # Contoh: angka 1-26
        try:
            predicted_char = chr(64 + int(predicted_number))  # 1->A, 2->B, ..., 26->Z
        except:
